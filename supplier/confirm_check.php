<?php
	...
	mysqli_query($conn,"insert into sales (userid, sales_total, sales_date, status, statuscust) values ('".$_SESSION['id']."', '$new', NOW(), 'Walk In', 'Order Complete')");
	$sid=mysqli_insert_id($conn);
	
	$query=mysqli_query($conn,"select * from cart where userid='".$_SESSION['id']."'");
	while($row=mysqli_fetch_array($query)){
		mysqli_query($conn,"insert into sales_detail (salesid, productid, size, sales_qty) values ('$sid', '".$row['productid']."', '".$row['size']."', '".$row['qty']."')");
